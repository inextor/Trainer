...
const CalendarGenerator = (() => {
  ...
  function makeDay(type, label, segs, vdot, plan) {
    return {
      ...
      distanceKm: segmentDistanceKm(segs, vdot),
  ...
  function stdWorkout(weekIdx, totalWeeks, event, daysPerWeek, vdot) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      if (!cell || cell.type === 'rest' || d >= daysPerWeek) {
        days.push({ type: 'rest', label: 'Rest', segments: [], totalMinutes: 0, distanceKm: 0, detail: '' });
        continue;
      }
      ...
      days.push({
        ...
        distanceKm: km,
        detail: `${Math.round(minutes)} min ${zone} (~${miles} mi)`
      });
    }
  ...
  function generateCalendar(config) {
    ...
    for (let k = 0; k < totalWeeks; k++) {
      ...
      if (event === 'marathon' && ['3', '4', '5'].includes(String(daysPerWeek)) && plans && plans.novice_marathon) {
        ...
        built = stdWorkout(k, totalWeeks, event, daysPerWeek, vdot);
      }
      // assign real dates
      ...
      const totalKm = days.reduce((t, d) => t + (d.distanceKm || 0), 0);
